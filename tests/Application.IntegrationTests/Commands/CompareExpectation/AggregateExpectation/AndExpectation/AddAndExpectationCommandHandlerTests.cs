...
namespace Application.IntegrationTests.Commands.CompareExpectation.AggregateExpectation.AndExpectation
{
    [Collection(nameof(TestCollections.ApplicationIntegrationCollection))]
    public class AddAndExpectationCommandHandlerTests : CommandTestBase
    {
        public AddAndExpectationCommandHandlerTests(ApplicationFixture applicationFixture) : base(applicationFixture)
        {
        }
        ...
        public async Task When_CommandIsInvalid_Expect_ValidationExceptionThrown()
        {
            ...
            {
                await mediator!.Send(new AddAndExpectationCommand
                {
        ...
        public async Task When_CommandIsValid_Expect_StepNavigatorGotExpectation()
        {
            ...
            var andExpectation = stepNavigatorFromDb.Expectations.First() as Domain.ProcessAggregate.Expectations.AggregateExpectations.AndExpectation;
            andExpectation!.Expectations.Count.Should().Be(2);
        }
    }
}